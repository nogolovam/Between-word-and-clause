...
path_all = 'C:/Users/nogol/Documents/Doktorat/Analyzy/SUD/vysledky260403/Characteristics/h_distance_vs_position_in_sentence/sort_by_mean_id'
...
def characteristics_units(a_treebank, output_folder, file_name):
    ...
    for sentence in a_treebank.sentence_list:
        if sentence.root_good and not sentence.bad_things and sentence.num_conj <= 1:
            ...
            if all_main_clauses:
                ...
                sorted_mc = sorted(all_main_clauses, key=lambda u: statistics.mean([w.id for w in u.word_list]))
                n_mc = len(sorted_mc)
                main_clause_data[n_mc]['parent_sizes'].append(n_mc)
                for pos, unit in enumerate(sorted_mc):
                    avg_v, med_v, max_v = get_h_distance_metrics(unit.word_list)
                    main_clause_data[n_mc][pos]['h_dist_avg'].append(avg_v)
                    main_clause_data[n_mc][pos]['h_dist_med'].append(med_v)
                    main_clause_data[n_mc][pos]['h_dist_max'].append(max_v)

            # --- CLAUSES ---
            if all_clauses:
                #sorted_cl = sorted(all_clauses, key=lambda u: min([w.id for w in u.word_list]))
                sorted_cl = sorted(all_clauses, key=lambda u: statistics.mean([w.id for w in u.word_list]))
                n_cl = len(sorted_cl)
                clause_data[n_cl]['parent_sizes'].append(n_cl)
                for pos, unit in enumerate(sorted_cl):
                    avg_v, med_v, max_v = get_h_distance_metrics(unit.word_list)
                    clause_data[n_cl][pos]['h_dist_avg'].append(avg_v)
                    clause_data[n_cl][pos]['h_dist_med'].append(med_v)
                    clause_data[n_cl][pos]['h_dist_max'].append(max_v)
    ...
    clause_data = merge_outliers(clause_data, w_clause)
