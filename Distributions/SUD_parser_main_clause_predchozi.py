...
def process_lds(a_clausal_head):
    ...
    clausal_lds = []
    current_lds = []
    clausal_nodes = list_non_clausal_children(a_clausal_head.direct_children, "clause")
...
def process_lds_phrase(a_phrase_head, list_of_nodes):
    """identifies a segment which words are linearly and syntactically connected"""
    ...
    # if a clausal head is the only clausal node
    if len(clausal_nodes) == 0:
        current_lds.append(a_phrase_head)
        clausal_lds.append(current_lds)
        return clausal_lds

    # if a clause includes more than one node
    clausal_nodes.append(a_phrase_head)  # a list of all clausal nodes including the clausal head
    clausal_nodes.sort(key=lambda word_node: word_node.id)  # sorts the list according to ID of nodes in the ascending order
    ...
    for i in range(len(clausal_nodes) - 1):
        ...
        try:
            ...
            print("AttributeError", clausal_nodes)
            continue
    ...
    def find_parent_and_children(self):  # NOTE: root parent is None!
        ...
        for sentence in self.sentence_list:
            for word in sentence.word_list:
                if word.parentID != 0 and word.deprel != 'punct' and word.parentID is not None:
...
class Sentence:
    ...
    def check_kompozita(self):
        with open('C:/Users/nogol/OneDrive - Ostravská univerzita/Dokumenty/Doktorat/Analyzy/MAL/kompozita_txt.txt', encoding="UTF-8", mode='r') as soubor:
            for radek in soubor:
                slova = radek.split(" ")
...
class Phrase:
    ...
    def __init__(self):
        ...
        self.big_chunk2_list: list[BigChunk2()] = []
    ...
    def get_big_chunk(self):
        ...
        for word in phrase_words:
            if word not in processed_words:
                ...
                new_big_chunk.get_LDS()
                new_big_chunk.get_chunk()
                new_big_chunk.get_LLB()



        for word2 in phrase_words2:
            if word2 not in processed_words2:
                new_big_chunk2 = BigChunk2()
                self._build_big_chunk2(word2, new_big_chunk2, processed_words2)
                self.big_chunk2_list.append(new_big_chunk2)
                new_big_chunk2.delka_big_chunk2 = len(new_big_chunk2.word_list)
                new_big_chunk2.get_strings_big_chunk()
                new_big_chunk2.get_root_word()
                new_big_chunk2.get_HD_values()
                new_big_chunk2.get_direct_children()
                new_big_chunk2.get_LDS()
                new_big_chunk2.get_chunk()
                new_big_chunk2.get_LLB()
    ...
    def _build_big_chunk2(self, word2, big_chunk2, processed_words2):
        """Recursively build a big chunk from the given word."""
        """we are cutting big chunk as in build_big_chunk, but also when the kid is conj"""
        big_chunk2.word_list.append(word2)
        processed_words2.add(word2)

        direct_children = word2.direct_children_phrase.copy()
        direct_children.sort(key=lambda word_node: word_node.id)
        #print("aktual word2: ", word2)

        if direct_children and direct_children[0].deprel not in {'conj', 'appos', 'conj@emb', 'conj:dicto', 'conj:coord', 'conj:appos'}:
            first_child = direct_children[0]
            if first_child not in processed_words2:
                if len(first_child.direct_children_phrase) <= 1:
                    self._build_big_chunk2(first_child, big_chunk2, processed_words2)
    #print("Phrase string made.")
...
class BigChunk:
    ...
    def get_LDS(self):
        ...
        list_nodes = self.word_list.copy()
        # print(list_nodes)
        list_nodes.remove(self.root_BigChunk)
        lds_list = process_lds_phrase(self.root_BigChunk, list_nodes)
        for lds_segment in lds_list:
            new_LDS = LDS()
            new_LDS.delka_LDS = len(lds_segment)
            new_LDS.word_list = lds_segment
            new_LDS.get_strings_LDS()
            self.LDS_list.append(new_LDS)
            self.pocet_LDS += 1
    ...
    def get_chunk(self):
        ...
        while current_distance >= self.min_HD_word:
            ...
            for parent_id, words in parent_to_words.items():
                ...
                valid_words.sort(key=lambda word_node: word_node.id)
                delete_words = []
                for i in range(len(valid_words) - 1):
                    if valid_words[i].id + 1 != valid_words[i + 1].id:
    ...
    def get_LLB(self):
        ...
        processed_words = set()
        phrase_words = self.word_list.copy()
        phrase_words.sort(key=lambda word_node: word_node.h_distance)  # sorts the list according to h_distance of nodes in the ascending order
        ...
        for word in phrase_words:
            if word not in processed_words:
                ...
                # Follow the chain of children with the smallest ID
                while current_word and current_word not in processed_words:
                    new_LLB.word_list.append(current_word)
                    processed_words.add(current_word)

                    # Find the child with the smallest ID
                    if current_word.direct_children_bigchunk:
                        current_word = min(current_word.direct_children_bigchunk, key=lambda child: child.id)
                    else:
                        current_word = None  # No more children, end the chain

                # Add the completed LLB to the list
                self.LLB_list.append(new_LLB)
                new_LLB.delka_LLB = len(new_LLB.word_list)
                new_LLB.get_strings_LLB()
                self.pocet_LLB += 1
...
class BigChunk2:
    ...
    def get_direct_children(self):
        ...
        for word in self.word_list:
            if len(word.direct_children) != 0:
                for child in word.direct_children:
                    if child in self.word_list:
                        word.direct_children_bigchunk2.append(child)
                for kid in word.all_children:
                    if kid in self.word_list:
                        word.all_children_bigchunk2.append(kid)
    ...
    def get_LLB(self):
        ...
        processed_words = set()
        phrase_words = self.word_list.copy()
        phrase_words.sort(key=lambda
            word_node: word_node.h_distance)  # sorts the list according to h_distance of nodes in the ascending order
        ...
        for word in phrase_words:
            if word not in processed_words:
                ...
                while current_word and current_word not in processed_words:
                    ...
                    # Find the child with the smallest ID
                    if current_word.direct_children_bigchunk2:
                        current_word = min(current_word.direct_children_bigchunk2, key=lambda child: child.id)
                    else:
                        current_word = None  # No more children, end the chain
...
def create_worddata(a_line):
    ...
    if new_word.deprel != "root" and new_word.deprel != "punct" and new_word.parentID is not None:
